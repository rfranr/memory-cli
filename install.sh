...
print_npm_install_command() {
  if [ -n "${PI_NPM_INSTALL_PREFIX:-}" ]; then
    printf 'npm install -g --ignore-scripts --prefix %s %s' "$PI_NPM_INSTALL_PREFIX" "$PI_PACKAGE"
  else
    printf 'npm install -g --ignore-scripts %s' "$PI_PACKAGE"
  fi
}
...
run_npm_install_pi() {
  npm_loglevel="$1"
  if [ -n "${PI_NPM_INSTALL_PREFIX:-}" ]; then
    npm install -g --ignore-scripts --prefix "$PI_NPM_INSTALL_PREFIX" --no-fund --no-audit "--loglevel=$npm_loglevel" --progress=false "$PI_PACKAGE"
  else
    npm install -g --ignore-scripts --no-fund --no-audit "--loglevel=$npm_loglevel" --progress=false "$PI_PACKAGE"
  fi
}
